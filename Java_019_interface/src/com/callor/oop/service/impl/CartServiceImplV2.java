...
public class CartServiceImplV2 implements CartService {
	/*
	 * 1. cartList와 scan 인스턴스 객체변수를 선언 "a만" 하고
	 * 생성은 하지 않는다.
	 * 
	 * 이렇게 하면 생성되지않은 인스턴스 객체변수의 method를 호출하는 코드에서
	 * NullpointerException 이 발생할 것이다
	 * 
	 * 3. NullpointerException을 방지하기 위하여
	 * 클래스 생상자 method에 객체 변수를 초기화 하는
	 * 코드를 반드시 추가해 준다.
	 * 
	 * 즉
	 ...
	 * 	혹시 모를 메모리 누수(leak)을 방지하기 위한 조치
	 ...
	 *  클래스 이름과 대소문자 까지 같은 method
	 *  return type(void, int,String 이 없는 method
	 *  
	 *  클래스를 객체로 선언하고, 초기화(생성)를 할때 사용하는 아래코드에서
	 *  CartServiceImplV2 cs = new CartServiceImplV2();
	 *  
	 *  new CartServiceImlpV2() 이부분의 코드가 
	 *  클래스의 생성자를 호출하여 객체를 생성하는 부분이다.
	 *  
	 *  생성자 method는 특별한 경우가 아니면 클래스를 선언할때 자동으로
	 *  코드가 만들어 지고 감춰진 상태로 존재한다.
	 */
